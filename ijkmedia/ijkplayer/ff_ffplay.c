...
long ffp_get_current_position_l_ex(FFPlayer *ffp, bool isStarted)
{
    assert(ffp);
    VideoState *is = ffp->is;
    if (!is || !is->ic)
...
//modefied by yangweiqing, fix bug #2173
	...
	if (isStarted) {
		for(; i <= 49; i++){
			...
			if (isnan(pos_clock)) {
		       pos = fftime_to_milliseconds(is->seek_pos);
				av_log(ffp, AV_LOG_DEBUG, "lhplayer ffp_get_current_position_l_ex ---sleep 10 ms and try again. i=%d\n", i);	
				usleep(10*1000);
			} else {
				pos = pos_clock * 1000;
				break;
		...
		if (i > 49) {
			av_log(ffp, AV_LOG_DEBUG, "lhplayer ffp_get_current_position_l_ex get_master_clock error will set is->seek_pos\n");
		}
	} else {
		av_log(ffp, AV_LOG_DEBUG, "lhplayer ffp_get_current_position_l_ex is not Started\n");
		if (isnan(pos_clock)) {
        	pos = fftime_to_milliseconds(is->seek_pos);
    	} else {
        	pos = pos_clock * 1000;
